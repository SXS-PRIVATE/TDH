...
class TrainBase(object):
    ...
    def __lr_scheduler(self):
        ...
        for optimizer in self.optimizers:
            scheduler = StepLR(optimizer, self.lr_decay_freq, self.lr_decay, verbose=True)
    ...
    def get_codes(img_model, txt_model, dataset, bit, batch_size, cuda=True):
        ...
        img_model.eval()
        txt_model.eval()
        for data in tqdm(dataloader, desc=f'get codes'):
